...
const animateCounter = () => {
  stats.forEach((stat, i) => {
    ...
    const timer = setInterval(() => {
      ...
      if (start >= end) {
        counters.value[i] = end;
        clearInterval(timer);
      } else {
        counters.value[i] = start;
      }
    }, stepTime);
  });
};
